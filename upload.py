...
def windows_stlink_attach() -> str:
    usb_id = None
    process = subprocess.run("usbipd.exe list", shell=True, capture_output=True)
    ...
    process = subprocess.run(f"usbipd.exe bind --busid {usb_id}", shell=True, capture_output=True)
    if process.returncode != 0:
        print(f"ERROR: Failed to bind the ST-Link. Command output: \n{process.stdout.decode('utf-8')}\n{process.stderr.decode('utf-8')}")
        exit(1)
    process = subprocess.run(f"usbipd.exe attach --wsl --busid {usb_id}", shell=True, capture_output=True)
    if process.returncode != 0:
...
def windows_stlink_detach(usb_id: str) -> None:
    process = subprocess.run(f"usbipd.exe detach --busid {usb_id}", shell=True, capture_output=True)
    if process.returncode != 0:
