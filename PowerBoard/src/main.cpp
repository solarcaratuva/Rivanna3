...
void signal_flash_handler() {
	...
	if (flashHazards) {
		...
		left_turn_signal.write(!left_turn_signal.read() & flashLeftTurnSignal);
		right_turn_signal.write(!right_turn_signal.read() & flashRightTurnSignal);
	}
	bms_strobe.write(!bms_strobe.read() & bms_error);
