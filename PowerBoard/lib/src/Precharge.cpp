...
void motor_precharge(){
    ...
    while (rc_voltage_motor.read_voltage() * HAL_EFFECT_SENSOR_SCALE < PACK_VOLT_PCT * pack_voltage) {
        ThisThread::sleep_for(SLEEP_TICK);
        fault_trap();
    }
...
void mppt_precharge(){
    ...
    while (rc_voltage_battery.read_voltage() * HAL_EFFECT_SENSOR_SCALE < PACK_VOLT_PCT * pack_voltage) {
        ThisThread::sleep_for(SLEEP_TICK);
        fault_trap();
    }
