 ...
 */
MotorInterface::MotorInterface(I2C &throttle_regen)
    : throttle_regen_bus(throttle_regen) {}
...
int MotorInterface::sendThrottle(uint16_t throttle) {
    ...
    int result = throttle_regen_bus.write(0x5E, cmd, 2);
    return result;
}
...
int MotorInterface::sendRegen(uint16_t regen) {
    ...
    int result = throttle_regen_bus.write(0x5C, cmd, 2);
    return result;
}
